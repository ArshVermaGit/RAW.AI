...
serve(async (req) => {
  ...
  try {
    ...
    if (userId) {
      ...
      const currentUsage = usageData?.reduce((sum, log) => sum + (log.words_count || 0), 0) || 0;
...
8. Avoid overused AI phrases like "dive into", "in conclusion", "it's important to note"`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        tool_choice: { type: "function", function: { name: "humanize_result" } }
      }),
    });
    ...
    if (!toolCall || toolCall.function.name !== "humanize_result") {
